...
@section('title', 'Grace & Ground Coffee Shop')
    ...
    <div class="container">
        ...
        <div style="margin-bottom: 70px;">
            ...
            <div class="products-grid">
                @foreach($categories->whereIn('slug', ['burnt-sugar', 'spanish-latte', 'mocha-latte']) as $category)
                <div class="product-card">
                    <div style="padding: 24px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <h4 style="font-size: 20px; color: var(--white); margin: 0;">{{ $category->name }}</h4>
                            <span style="color: var(--royal-gold); font-weight: 600;">from ₱140</span>
                        </div>
                        <a href="{{ route('products.category', $category->slug) }}" style="color: var(--royal-gold); font-size: 14px; text-decoration: none;">View drinks →</a>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div style="margin-bottom: 70px;">
            ...
            <div class="products-grid">
                @foreach($categories->whereIn('slug', ['strawberry-matcha', 'matcha-cloud', 'choco-heaven']) as $category)
                <div class="product-card">
                    <div style="padding: 24px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <h4 style="font-size: 20px; color: var(--white); margin: 0;">{{ $category->name }}</h4>
        ...
        <div style="margin-bottom: 70px;">
            ...
            <div class="products-grid">
                @foreach($categories->whereIn('slug', ['lychee-soda', 'strawberry-soda', 'blueberry-soda']) as $category)
                <div class="product-card">
                    <div style="padding: 24px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <h4 style="font-size: 20px; color: var(--white); margin: 0;">{{ $category->name }}</h4>
        ...
        <div>
            ...
            <div class="products-grid">
                @foreach($categories->whereIn('slug', ['fries-overload', 'nachos', 'burger']) as $category)
                <div class="product-card" style="opacity: {{ $category->is_active ? 1 : 0.5 }};">
                    <div style="padding: 24px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <h4 style="font-size: 20px; color: var(--white); margin: 0;">{{ $category->name }}</h4>
                            <span style="color: var(--royal-gold); font-weight: 600;">{{ $category->is_active ? 'from ₱120' : 'Coming Soon' }}</span>
                        </div>
                        @if($category->is_active)
                        <a href="{{ route('products.category', $category->slug) }}" style="color: var(--royal-gold); font-size: 14px; text-decoration: none;">View →</a>
                        @else
        ...
        <div class="products-grid">
            ...
            <div class="product-card">
                ...
                <div style="padding: 20px;">
                    <span style="color: var(--royal-gold); font-size: 12px; font-weight: 600; text-transform: uppercase;">{{ $product->category?->name ?? 'Coffee' }}</span>
                    <h4 style="font-size: 20px; color: var(--white); margin: 8px 0;">{{ $product->name }}</h4>
                    <p style="color: var(--white-muted); font-size: 14px; margin-bottom: 15px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ $product->description }}</p>
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <span style="font-size: 22px; color: var(--white); font-weight: 600;">₱{{ number_format($product->price, 0) }}</span>
    ...
    <div class="container">
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 60px; align-items: center;">
            ...
            <div style="background: var(--dark-card); border-radius: 20px; padding: 40px; text-align: center;">
                ...
                <a href="{{ route('products.index') }}" class="btn btn-gold" style="display: inline-block; padding: 15px 40px; border-radius: 30px; text-decoration: none;">Order Now</a>
            </div>
        </div>
    </div>
</section>
